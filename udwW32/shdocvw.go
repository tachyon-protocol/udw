...
func (wb2 *IWebBrowser2) Refresh2() HRESULT {
	ret, _, _ := syscall.Syscall(wb2.LpVtbl.Refresh2, 1,
		uintptr(unsafe.Pointer(wb2)),
		0,
		0)
...
func (wb2 *IWebBrowser2) Get_Document(ppDisp **IDispatch) HRESULT {
	ret, _, _ := syscall.Syscall(wb2.LpVtbl.Get_Document, 2,
		uintptr(unsafe.Pointer(wb2)),
		uintptr(unsafe.Pointer(ppDisp)),
		0,
	)
	return HRESULT(ret)
}
