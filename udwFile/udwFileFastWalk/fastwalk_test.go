...
func testFastWalk(t *testing.T, files map[string]string, callback func(path string, typ os.FileMode) error, want map[string]os.FileMode) {
	...
	var mu sync.Mutex
	if err := udwFileFastWalk.Walk(tempdir, func(path string, typ os.FileMode) error {
		mu.Lock()
		defer mu.Unlock()
	...
	testFastWalk(t, map[string]string{
		...
		func(path string, typ os.FileMode) error {
			if strings.HasSuffix(path, "_skipfiles.go") {
				...
				return udwFileFastWalk.SkipFiles
			}
			return nil
		},
		want)
	...
	testFastWalk(t, map[string]string{
		...
		func(path string, typ os.FileMode) error {
			if typ == os.ModeSymlink {
				return udwFileFastWalk.TraverseLink
			}
			return nil
		},
		map[string]os.FileMode{
...
func BenchmarkFastWalk(b *testing.B) {
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		err := udwFileFastWalk.Walk(*benchDir, func(path string, typ os.FileMode) error { return nil })
		if err != nil {
			b.Fatal(err)
		}
	}
