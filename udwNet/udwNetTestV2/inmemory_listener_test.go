...
func TestInmemoryListenerBench(t *testing.T) {
	ln := NewInmemoryListener()

	serverReadBuf := make([]byte, 30)
	serverWriteContent := bytes.Repeat([]byte{1}, 30)
	closer := udwNet.TcpNewListenerFromExistListener(ln, func(conn net.Conn) {
		conn.Read(serverReadBuf)
		conn.Write(serverWriteContent)
		conn.Close()
	})
	defer closer()
	clientReadBuf := make([]byte, 30)
	runOnce := func() {
		conn, _ := ln.Dial()
		conn.Write(serverWriteContent)
		conn.Read(clientReadBuf)
		conn.Close()
	}
	for i := 0; i < 100; i++ {
		runOnce()
