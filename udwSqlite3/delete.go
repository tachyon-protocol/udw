...
func (db *Db) MustDelete(k1 string, k2 string) {
	errMsg := db.querySkipResult(`DELETE FROM `+db.getEscapedTableName(k1)+` WHERE k=?`, []byte(k2))
	if errMsg != "" {
...
func (db *Db) mustMulitDeleteL1(k1 string, k2List []string) {
	sqlBuf := bytes.NewBufferString(`DELETE FROM ` + db.getEscapedTableName(k1) + ` WHERE k IN (`)
	valueBuf := [][]byte{}
	l := len(k2List)
...
func (db *Db) MustEmptyK1(k1 string) {
	db.mustSetExec(k1, `DELETE FROM `+db.getEscapedTableName(k1), nil)
}
