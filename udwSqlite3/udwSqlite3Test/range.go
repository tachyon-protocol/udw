...
func TestMustGetRange() {
	MustRunTestDb(func(db *udwSqlite3.Db) {
		...
		ret := db.MustGetRange(udwSqlite3.MustGetRangeRequest{
			K1: "test",
		})
		...
		sList := db.MustGetRangeValueList(udwSqlite3.MustGetRangeRequest{
			K1: "test",
		})
		udwTest.Equal(len(sList), 3)
		udwTest.Equal(sList, []string{"v1", "v2", "v3"})

		sList = db.MustGetRangeKeyList(udwSqlite3.MustGetRangeRequest{
			K1: "test",
		})
		udwTest.Equal(sList, []string{"1", "2", "3"})

		keyValuePairList := db.MustMulitGet("test", []string{"1", "2"})
		udwTest.Equal(keyValuePairList, []udwSqlite3.KeyValuePair{
			{Key: "1", Value: "v1"},
			{Key: "2", Value: "v2"},
		})
...
func TestGetRange2() {
	...
	defer db.Close()
	ret := db.MustGetRangeValueList(udwSqlite3.MustGetRangeRequest{
		K1: "test",
	})
	udwTest.Equal(len(ret), 0)
	keyList := db.MustGetRangeKeyList(udwSqlite3.MustGetRangeRequest{
		K1: "test",
	})
	...
	m := db.MustGetRangeKeyMap(udwSqlite3.MustGetRangeRequest{
		K1: "test",
	})
	udwTest.Equal(len(m), 2)
	m2 := db.MustGetRangeToMap(udwSqlite3.MustGetRangeRequest{
		K1: "test",
	})
