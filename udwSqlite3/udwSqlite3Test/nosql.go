...
func TestSet() {
	MustRunTestDb(func(db *udwSqlite3.Db) {
		...
		task.Close()
		list := db.MustGetRange(udwSqlite3.GetRangeReq{
			K1: "test",
		})
...
func TestMustMulitDelete() {
	...
	keyList := []string{}
	keyValueList := []udwMap.KeyValuePair{}
	for i := 0; i < 100; i++ {
		k := strconv.Itoa(i)
		keyList = append(keyList, k)
		keyValueList = append(keyValueList, udwMap.KeyValuePair{
			Key:   k,
			Value: k,
		})
	}
	...
	outList := db.MustGetRange(udwSqlite3.GetRangeReq{
		K1: "test",
	})
